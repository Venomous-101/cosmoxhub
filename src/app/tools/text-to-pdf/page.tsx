...
import {
  ...
  AlignCenter, AlignRight, Underline
} from "lucide-react";
...
export default function TextToPdfPage() {
  ...
  // Simple rich-text formatting state for the textarea approach
  // We parse **bold**, *italic*, __underline__ markers
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      for (const rawLine of lines) {
        ...
        // Word-wrap: build words with their segment info
  ...
  return (
    <ToolLayout
      title="Text to PDF Elite"
      description="Convert styled text into professional PDFs. Supports **Bold**, *Italic*, and __Underline__ formatting. 100% private, zero uploads."
      icon={FileText} color="#3b82f6"
    >
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        "@context": "https://schema.org", "@type": "SoftwareApplication",
        "name": "CosmoxHub Elite Text to PDF Generator",
        "operatingSystem": "Any", "applicationCategory": "Utility",
        "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" },
        "featureList": ["Bold/Italic/Underline formatting", "Multiple font families", "Alignment control", "Multi-page auto flow", "100% Client-side privacy"]
      })}} />
            ...
            <div className="px-8 py-3 border-b border-white/[0.03] flex items-center gap-6 bg-blue-500/[0.02]">
              {[{ tag: "**bold**", icon: Bold }, { tag: "*italic*", icon: Italic }, { tag: "__underline__", icon: Underline }].map(h => (
                <span key={h.tag} className="flex items-center gap-1.5 text-[9px] text-slate-600 font-mono">
                  <h.icon size={10} className="text-blue-500/40" /> {h.tag}
                </span>
              ))}
            </div>
