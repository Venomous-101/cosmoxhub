...
export default function JPGToPNGPage() {
  ...
  return (
    <ToolLayout
      title="Elite JPG to PNG Converter"
      description="Convert JPG images to lossless PNG format in bulk. Zero quality loss, transparency-ready output, 100% private client-side processing."
    ...
    >
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "featureList": ["Lossless PNG output", "Bulk processing", "No signup required", "100% Client-side", "Privacy focused"]
      })}} />
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-8 items-start">
        ...
        <aside className="space-y-5 sticky top-8">
          ...
          <div className="bg-[#0a0a1a]/40 border border-white/5 rounded-3xl p-6 space-y-4">
            <h4 className="text-white font-bold text-sm flex items-center gap-2"><LayoutGrid size={14} className="text-emerald-500" /> Why PNG?</h4>
            {[
              { t: "Lossless Quality", d: "PNG preserves every pixel exactly from the source image." },
              { t: "Transparency Support", d: "PNG supports alpha channels — ideal for logos and UI assets." },
              { t: "100% Private", d: "Everything runs in your browser. Zero uploads." },
            ].map(tip => (
              <div key={tip.t} className="space-y-0.5">
