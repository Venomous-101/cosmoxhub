...
import { preloadFFmpegWasm } from "@/lib/ffmpeg-preloader";
 ...
 *
 * EXTENDED: Also preloads FFmpeg WASM when user hovers the Video to GIF card,
 * so the engine starts downloading before the user even clicks the tool.
 */
    ...
    const handleMouseOver = (e: MouseEvent | TouchEvent) => {
      ...
      if (anchor && anchor.href) {
        ...
        if (url.hostname === currentDomain || url.pathname.startsWith("/")) {
          ...
          // 🎯 FFmpeg WASM Preload — triggers only on Video to GIF card hover
          // Downloads WASM files into browser cache before user even clicks.
          // preloadFFmpegWasm() is idempotent — safe to call many times.
          if (url.pathname === "/tools/video-to-gif") {
            preloadFFmpegWasm();
          }
        }
      }
